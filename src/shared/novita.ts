...
export const NOVITA: Novita[] = [
  ...
  },
  {
    versione: '0.27.0',
    righe: [
      '**Le chat di un altro PC non si aprono più in una cartella vuota.** Una chat arrivata dal Drive porta con sé la cartella del PC in cui lavora; se qui quella cartella non c’era, SierraDeck la «adottava»: creava una cartella vuota con lo stesso nome in «Progetti SierraDeck», ci spostava la conversazione e Claude ripartiva senza i file del progetto: «directory non trovata» e righe rosse a ogni comando, e da lì in poi la copia vera dell’altro PC non arrivava più (le due copie divergevano in silenzio). Ora ogni PC lascia sul Drive, con il suo battito, le cartelle che ha; una chat la cui cartella ce l’ha un altro PC resta sua: nel riquadro compare «Questa chat lavora su «Portatile»», con «Scrivile là» (la posta: quel PC la esegue nella sua chat quando è acceso, con la cartella e la conversazione già scelte) e «Aprila qui lo stesso» (in una cartella vuota, sapendolo). Le chat già adottate per sbaglio tornano da sole sotto la cartella del loro PC, e la copia più avanti dell’altro PC arriva anche se qui la stessa chat sta sotto un’altra cartella. Con la chat si sposta anche la cartella dei subagenti. Dal telefono, in «Riprendi una conversazione», una chat di un altro PC dice «su X» e non si riapre qui: si scrive da «Altri PC». Nel registro dei progetti lo stesso progetto adottato da due PC diventa uno. Finché l’altro PC non ha questa versione, una cartella con chat toccate sul Drive negli ultimi sette giorni conta come viva su un altro PC.',
      '**Parla con l’autopilota, fuori dalla sua chat.** Nella scheda di ogni autopilota (sul PC, nella pagina del telefono e nell’app Android) c’è «Parla con lui»: gli scrivi e lui risponde con parole sue, con davanti obiettivo, criteri, diario e l’ultima cosa scritta dalla chat che governa. Se quello che scrivi è un’istruzione la applica: cambia obiettivo o criteri, aggiunge un compito, si ferma («fermati»), riparte («riprendi»), risponde a una sua domanda aperta. Se serve che la chat lo sappia, glielo consegna alla fine del turno che ha in mano, mai in mezzo a un’azione, oppure appena riparte se è fermo: nella chat compare come «Chi ti ha dato il compito ti scrive: «…»», davanti alle istruzioni del turno, e il supervisore lo vede nel quadro prima di decidere. La risposta arriva di solito entro qualche minuto (intanto la scheda dice «sta pensando»); la conversazione resta nell’archivio dell’autopilota, si vede nella scheda e nel diario, distinta dalla chat. La vecchia casella «digli cosa cambiare» è dentro questa.',
      '**Un autopilota apre sempre una chat sua.** Il 13 settembre il primo mandato dell’autopilota è finito dentro la chat che stavi usando nella stessa cartella: la consegna «adottava» la prima chat libera sulla stessa cartella e nello stesso workspace, le uccideva il terminale e lo faceva rinascere con i suoi hook. Non succede più: se la sessione decisa dal servizio non ha un riquadro, se ne apre uno nuovo nel workspace da cui l’autopilota è stato avviato. La tua chat resta tua.',
      '**Azioni su un altro PC, che si eseguono solo là quando è acceso.** Nel pannello Account, «Altri computer»: ogni PC che usa questo Drive lascia un battito (nome, quando, le cartelle in cui lavora, le chat aperte). «Azioni…» apre la sua cassetta sul Drive: quello che ci scrivi si esegue solo su quel PC, nella cartella che scegli, quando è acceso: il suo postino passa ogni mezzo minuto e consegna alla prima chat di quella cartella che ha finito e aspetta, o alla chat che indichi, o ne apre una nuova; se la cartella là non esiste la voce fallisce e lo dice. È la strada per una cartella che sta su quel PC e non viaggia sul Drive (un disco di rete montato solo là, un progetto locale). Il lavoro fatto lo vedi come sempre: la chat sale sul Drive e arriva qui, e dal telefono guardi dentro la chat di quel PC. Anche dal telefono (pagina e app: «Altri PC»). Le due cose piccole degli autopiloti: riprendere un autopilota finito riparte davvero, e «Mandaglielo» non fallisce più dopo tre secondi.',
      '**Workspace tanti, un menu.** Con più di sei workspace le linguette in alto diventavano puntini («Prede…», «W…»): ora la fascia mostra il nome intero di dove sei e un menu che li elenca tutti per esteso, con quante chat ha ciascuno, il pallino di chi aspetta una tua risposta, e una casella per cercare quando sono più di otto (si scrive un pezzo del nome e Invio). Fino a sei restano le linguette. **La scheda dell’autopilota scorre**: prima una scheda alta restava tagliata senza barra. **Le righe a scalino**: dopo una striscia o un pannello che cambiava la misura del riquadro, Claude Code si ridisegnava sulle colonne sbagliate; ora il terminale prende una misura sola, quando il riquadro è fermo. **La sincronia non sposta più l’impaginazione.** Il lavoro con il Drive non sta più in una striscia in alto che entrava e usciva ogni cinque minuti spostando i riquadri: ora sono fumetti in basso a destra, fuori dal flusso, che compaiono e spariscono senza muovere niente. Il lavoro che chiedi tu (Fondi, Ripristina, Porta qui) ha il suo fumetto con la barra, «Dettagli» e «Annulla»; il salvataggio automatico e l’arrivo delle chat non si vedono affatto, se ne parla solo se vanno male (nelle Impostazioni puoi accendere una riga piccola con la percentuale). Le chat arrivate e il riavvio consigliato sono fumetti, e quelli buoni si chiudono da soli. **Drive: «Togli il workspace dal Drive».** Nella scheda Drive, vista «Per workspace», ogni workspace ha «Togli dal Drive»: sparisce dall’archivio sul Drive e ci resta una lapide, così nessun PC lo rimette con il salvataggio automatico e non ricompare con «Fondi» né con «Porta qui». Le sue chat restano sul Drive e sui PC; sui PC il workspace resta com’è. L’elenco «Tolti dal Drive» ha «Rimetti sul Drive». **«Ripristina dal Drive» non porta più qui le impostazioni e le istantanee dell’altro PC** (si scaricano solo se qui mancano) e prima fa una copia `*.prima-del-ripristino-drive.json` dei tre file dell’assetto. Le sessioni «observer» del plugin claude-mem non stanno più nell’elenco Chat e non viaggiano sul Drive. Un salvataggio automatico o un arrivo che fallisce si vede nella striscia in alto.',
      '**Progetti: «Prendi il testimone» o «Ripristina» su un progetto tolto dal Drive da un altro PC non cancella più la cartella di qui** (.git compreso): senza file sul Drive non si tocca niente e lo si dice. Le chat con una cartella che qui non c’è: una sottocartella sparita di un progetto già tuo si ricrea lì, un progetto conosciuto ma mai collegato qui si collega subito, due progetti omonimi non finiscono nella stessa cartella, le cartelle temporanee (Temp, AppData) non diventano progetti. L’elenco Chat non cambia più cartella a una chat che sta sotto due cartelle a ogni rilettura.',
      '**Aggiornamenti: se SierraDeck Update non parte, tutto torna com’era** (autopiloti fuori dalla pausa, chat libere, installazione alla chiusura ancora valida) e la striscia dice perché. La fase «aspetto che le chat finiscano» ha la sua striscia sul PC, con cosa succede e quanto dura. Riavviare (dal PC o dal telefono) con un aggiornamento pronto lo installa invece di far ripartire la versione vecchia sotto l’installer. L’updater ora vede davvero la versione di Claude Code (prima `npm.cmd` falliva in silenzio). Le chat usano il fornitore alternativo impostato nelle preferenze (prima l’ambiente si perdeva per strada e parlavano sempre con Anthropic). Chiusura più sicura: un lavoro con il Drive in corso si annulla in modo ordinato prima di uscire (prima «Esci» durante una fusione la uccideva a metà); un `index.db` tenuto da un antivirus non viene più cancellato; la chiusura, il PTY host e un avvio fallito scrivono nel registro. Rinominare un workspace non lo sdoppia più al primo salvataggio; eliminarne uno chiede conferma, dice cosa fa e mette da parte una copia; un workspace eliminato da un’altra finestra spegne i suoi terminali anche qui.',
      '**Telefono, negozio, quaderno, registro.** App Android allineata alla pagina: un comando che il computer rifiuta compare in cima con la sua spiegazione (prima moriva in silenzio); un autopilota «pronto» sta fra le urgenze con il tasto «Vai»; il dettaglio di un autopilota fermo dice perché; accanto a ogni criterio l’esito dell’ultima verifica («passato» o la prima riga dell’uscita); «N interventi»; le decisioni con l’ora e senza la sigla interna; «raggiunto alle HH:MM»; una chat di un altro PC nell’elenco «Riprendi» dice «su X». Toccare un autopilota, aprire le cartelle, la coda, il Drive o le impostazioni cambiava la pagina solo quando una chat stava scrivendo (era lei a far ridisegnare tutto): a PC fermo il dettaglio non compariva e il Drive restava su «Leggo il Drive…». Ora ogni cosa letta apposta entra nell’impronta della pagina. E ancora, sul telefono: «Elimina» e «Riparte al riavvio» anche nella pagina; «Quaderno» dal dettaglio dell’autopilota apre la cartella giusta; un comando che il computer rifiuta si vede in cima alla pagina; notifiche per «X aspetta te» e «ha finito»; la coda condivisa si rilegge da sola; le cartelle proposte sono quelle vere (prima `Game_ascensore` diventava `Game\ascensore`). App Android: il LED come lo decide il PC, «si prepara», la delega dice perché non parte. il negozio trova la cartella in ~/.claude.json anche se scritta con le barre diverse; «claude.exe non trovato» si dice per esteso; due schede del quaderno con lo stesso titolo nello stesso giorno non si sovrascrivono; il registro cancella da solo i giorni oltre due settimane; «salvate» nelle impostazioni solo se il file è stato scritto.'
    ]
  },
  {
    versione: '0.26.0',
    righe: [
      '**La sincronizzazione va nei due versi: le chat degli altri PC arrivano da sole.** Fino a ieri il salvataggio automatico solo caricava; per vedere qui le chat del portatile bisognava aprire ☁ Drive e premere «Porta qui» o «Fondi». Ora, dopo ogni salvataggio automatico, parte un lavoro «Arrivo dal Drive» che scarica le chat che stanno solo sul Drive o che di là sono più avanti (per contenuto: una chat cresce e basta, quindi la più lunga è la più aggiornata). Mai i file delle cartelle dei progetti (quelli restano a «Porta qui» e «Fondi»), mai una cancellazione, mai una chat di qui accorciata. Lo vedi nella striscia in alto come ogni lavoro con il Drive, puoi annullarlo, e se non c’è niente da scaricare non parte nemmeno. Alla chiusura del programma si sale soltanto.',
      '**Le chat arrivate compaiono subito nell’elenco, senza riavviare.** Dopo ogni lavoro che ha portato giù qualcosa (arrivo, fusione, ripristino, «Porta qui») l’indice delle chat viene riletto e in alto compare «N chat arrivate dal Drive» con il tasto «Apri l’elenco». Il 13 settembre una fusione aveva scaricato 532 chat sul fisso e l’elenco era rimasto quello di prima fino al riavvio: non succede più.',
      '**Le chat nate su un altro PC vanno nelle cartelle di qui, tutte insieme.** Una chat porta dentro la cartella del PC in cui è nata (`C:\\Users\\…\\Wdeck` del portatile): sul fisso quella cartella non c’è. Ora, all’avvio e dopo ogni arrivo, tutte le chat con una cartella che qui non esiste vengono spostate nella cartella di qui (il progetto già noto per quel percorso, oppure `Documenti\\Progetti SierraDeck\\<nome>` creato apposta, con l’origine ricordata), con la trascrizione riscritta in modo che Claude Code la ritrovi. Nell’elenco Chat le vedi sotto la cartella di qui, non sotto quella dell’altro PC, e si aprono. Le cartelle nascoste degli strumenti (per esempio `.claude-mem`) non si toccano. La cartella nuova è vuota: il codice arriva da git o da «Porta qui» se il progetto viaggia sul Drive.',
      '**I workspace di un PC non cancellano più quelli dell’altro sul Drive.** Sul Drive c’è un solo archivio dei workspace per tutti i PC, e ogni salvataggio lo sovrascriveva con il proprio: nel registro compariva «SALVA conflitto su workspaces.json: vince questo PC» a ogni giro, e sull’altro PC «Fondi» e «Porta qui il workspace» non trovavano più niente. Ora sul Drive sale l’unione dei workspace di tutti i PC; quello che vedi qui restano i tuoi workspace, e prendi gli altri dalla scheda Drive quando vuoi. Le impostazioni e le istantanee riscritte dall’altro PC non contano più come conflitto: sono file di ogni PC.',
      '**Nel registro** le righe «ARRIVO: N chat da portare qui», «N chat rimappate nelle cartelle di qui» e «la cartella … qui non c’è: la chat … lavora in …» raccontano cosa è successo e dove.'
    ]
  },
  {
    versione: '0.25.2',
  ...
  },
  {
    ...
    righe: [
      ...
      '**Lo stesso sul telefono**, nella sezione Drive del tab Computer: la barra con la fase e la percentuale mentre il computer legge, al posto di «Leggo il Drive…» e basta. Pagina servita e app Android (2.28.0).',
      '**Nel registro** (Impostazioni → Registro) ogni lettura del Drive dice quanto ha preso: «CATALOGO letto in N ms», con quanti progetti e chat.'
    ]
  },
  {
    versione: '0.24.1',
