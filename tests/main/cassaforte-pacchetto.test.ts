...
describe('pacchetto (formato binario)', () => {
  it('fa il giro completo con contenuto anche binario e percorsi unicode', async () => {
    const voci = [
    ...
    const blocco = await componiPacchetto(voci, '2026-08-24T10:00:00.000Z')
    const letto = await leggiPacchetto(blocco)
  ...
  it('regge un contenuto grosso senza «invalid string length»', async () => {
    ...
    const blocco = await componiPacchetto([{ percorso: 'chat/grande.jsonl', contenuto: grosso }], '2026-08-24T10:00:00.000Z')
    const letto = await leggiPacchetto(blocco)
    expect(letto?.voci[0]?.contenuto.length).toBe(grosso.length)
    expect(letto?.voci[0]?.contenuto.equals(grosso)).toBe(true)
  })

  it('un pacchetto vuoto è valido', async () => {
    const letto = await leggiPacchetto(await componiPacchetto([], '2026-08-24T10:00:00.000Z'))
    expect(letto?.voci.length).toBe(0)
    expect(letto?.creatoIl).toBe('2026-08-24T10:00:00.000Z')
  })

  it('un blocco che non è un pacchetto nostro si rifiuta, senza sollevare', async () => {
    expect(await leggiPacchetto(Buffer.from('non sono gzip'))).toBeUndefined()
    // gzip valido ma contenuto senza il nostro marcatore.
    expect(await leggiPacchetto(gzipSync(Buffer.from('ciao')))).toBeUndefined()
  })
})
