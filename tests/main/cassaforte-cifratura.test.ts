...
describe('la cassaforte end-to-end', () => {
  it('cifra e decifra un giro completo con la chiave-maestra', () => {
    const { maestra } = creaCassaforte('una-passphrase-robusta')
    const blocco = cifra(maestra, SEGRETO)
    // Il cifrato non contiene il testo in chiaro.
    expect(blocco.includes(Buffer.from('chiave API'))).toBe(false)
    expect(decifra(maestra, blocco)?.toString('utf8')).toBe(SEGRETO.toString('utf8'))
  })

  it('la passphrase giusta ritrova la stessa chiave-maestra, quella sbagliata no', () => {
    ...
    expect(decifra(riaperta!, cifra(maestra, SEGRETO))?.toString('utf8')).toBe(SEGRETO.toString('utf8'))
  ...
  it('cambiare passphrase ri-avvolge la stessa maestra: i dati restano, il recupero pure', () => {
    const nuova = creaCassaforte('vecchia-parola')
    const blocco = cifra(nuova.maestra, SEGRETO)
    ...
    expect(decifra(conNuova!, blocco)?.toString('utf8')).toBe(SEGRETO.toString('utf8'))
    // E il recupero resta valido.
    expect(stessaMaestra(sbloccaConRecupero(dopo, nuova.chiaveRecupero)!, nuova.maestra)).toBe(true)
  })

  it('un blocco manomesso non si decifra: GCM lo rifiuta invece di indovinare', () => {
    ...
    expect(decifra(maestra, rovinato)).toBeUndefined()
  })
